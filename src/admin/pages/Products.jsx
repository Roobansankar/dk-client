      ...
      {loading ? (
        ...
        <div className="card">
          <EmptyState
            ...
            description="Add a product with a photo, MRP and selling price."
            action={
              can('products.create') && (
                <Button size="sm" onClick={() => setModal({ mode: 'create' })}>
                  <Plus size={15} /> New product
            ...
            <article key={product.id} className="card flex flex-col overflow-hidden">
              <div className="relative aspect-[4/3] bg-[var(--color-surface-sunken)]">
                <Thumb
                  ...
                  alt={product.name}
                  className={cn('h-full w-full', !product.status && 'opacity-40 grayscale')}
                />
                {!product.status && (
              ...
              <div className="flex flex-1 flex-col p-3.5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="min-w-0 font-medium text-[var(--color-ink)]">
                    {product.name}
                  </h3>
                  <Pill tone={product.gst_inclusive ? 'neutral' : 'warn'}>
                    {product.gst_inclusive ? 'Incl. GST' : '+ GST'}
                  </Pill>
                </div>
                ...
                <div className="mt-auto flex items-center justify-between border-t border-[var(--color-line)] pt-3">
                  {can('products.update') ? (
                    <Toggle
                      ...
                      onChange={() => toggleMut.mutate(product)}
      ...
      <form className="flex flex-col gap-4" onSubmit={submit}>
        ...
        <div className="flex items-center justify-between border-t border-[var(--color-line)] pt-4">
          ...
          <Toggle
            ...
            onChange={(v) => setForm((f) => ({ ...f, gst_inclusive: v }))}
          />
        </div>
